...
PORT=8787 pnpm --filter @verge/api dev >/tmp/verge-api.log 2>&1 &
api_pid=$!
...
manual_response="$(
  curl -sf \
    -X POST \
    http://127.0.0.1:8787/run-requests/manual \
...
VERGE_API_URL=http://127.0.0.1:8787 pnpm --filter @verge/worker dev >/tmp/verge-worker.log 2>&1 &
worker_pid=$!

for _ in $(seq 1 180); do
  detail="$(curl -sf http://127.0.0.1:8787/run-requests/$manual_run_request_id)"
...
manual_detail="$(curl -sf http://127.0.0.1:8787/run-requests/$manual_run_request_id)"
...
resume_sha="resume-$(date +%s)-$head_sha"
resume_seed_response="$(
  curl -sf \
    -X POST \
    http://127.0.0.1:8787/run-requests/manual \
    -H "content-type: application/json" \
    --data "{\"repositorySlug\":\"verge\",\"commitSha\":\"$resume_sha\",\"requestedProcessSpecKeys\":[\"test\"],\"resumeFromCheckpoint\":false}"
)"
...
VERGE_API_URL=http://127.0.0.1:8787 pnpm --filter @verge/worker dev -- --once >/tmp/verge-resume-seed-worker.log 2>&1

seed_detail="$(curl -sf http://127.0.0.1:8787/run-requests/$resume_seed_request_id)"
seed_run_id="$(node -e "const data = JSON.parse(process.argv[1]); process.stdout.write(data.runs[0].id);" "$seed_detail")"
seed_run_detail="$(curl -sf http://127.0.0.1:8787/runs/$seed_run_id)"
...
resume_response="$(
  curl -sf \
    ...
    http://127.0.0.1:8787/run-requests/manual \
    -H "content-type: application/json" \
    --data "{\"repositorySlug\":\"verge\",\"commitSha\":\"$resume_sha\",\"requestedProcessSpecKeys\":[\"test\"],\"resumeFromCheckpoint\":true}"
)"
resume_request_id="$(node -e "const data = JSON.parse(process.argv[1]); process.stdout.write(data.runRequestId);" "$resume_response")"
resume_request_detail="$(curl -sf http://127.0.0.1:8787/run-requests/$resume_request_id)"
resume_run_id="$(node -e "const data = JSON.parse(process.argv[1]); process.stdout.write(data.runs[0].id);" "$resume_request_detail")"
resume_run_detail="$(curl -sf http://127.0.0.1:8787/runs/$resume_run_id)"
...
VERGE_API_URL=http://127.0.0.1:8787 pnpm --filter @verge/worker dev >/tmp/verge-resume-worker.log 2>&1 &
resume_worker_pid=$!

for _ in $(seq 1 180); do
  detail="$(curl -sf http://127.0.0.1:8787/run-requests/$resume_request_id)"
  status="$(node -e "const data = JSON.parse(process.argv[1]); process.stdout.write(data.status);" "$detail")"
  if [[ "$status" == "completed" ]]; then
    break
  fi
...
resume_final="$(curl -sf http://127.0.0.1:8787/run-requests/$resume_request_id)"
...
const summary = {
  ...
  selfHostedRuns: manual.runs.map((run) => ({ key: run.processSpecKey, status: run.status })),
